...
class DuAnKhachHangController extends Controller
{
    ...
    public function show_quyen_thanhvien(Request $request)
    {
        if($request->get('api_token'))
        {
            ...
            if($validate)
            {
                ...
                $id_du_an_kh = $request->get('id_du_an_kh');
                $quyen = DB::SELECT("SELECT * FROM TB_QUYEN_DA WHERE id_nd = $id_nd and id_du_an = $id_du_an_kh");
    ...
    public function show(Request $request,$id){
        if($request->has('api_token'))
        {
            $du_an = DB::SELECT("SELECT q_xem,q_them,q_sua,q_xoa,q_xuat FROM TB_DU_AN_KH WHERE ID_DU_AN_KH = $id");
            return response()->json($du_an, 200);
        }
    }
